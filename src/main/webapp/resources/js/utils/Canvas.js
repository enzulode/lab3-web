...
class Canvas {
	...
	drawSemiCircle(R, color = "#0098fa30") {
		...
		ctx.fillStyle = color
		ctx.beginPath()
		ctx.arc(canvas.width / 2, canvas.height / 2, R * this.dynamicScaling, Math.PI, Math.PI / 2, true)
	...
	drawTriangle(R, color = "#0098fa30") {
		...
		ctx.lineTo(canvas.width / 2, canvas.height / 2 - R / 2 * this.dynamicScaling)
	...
	drawRectangle(R, color = "#0098fa30") {
		let { ctx, canvas } = this
	    ctx.fillRect(canvas.width / 2, canvas.height / 2, R * this.dynamicScaling, R * this.dynamicScaling);
	}
	...
	applyPoints(points) {
		for (let i = 0; i < points.length; i++) {
			let displayed = this.currentR !== 0 && this.currentR === points[i].r

			if (displayed)
				this.drawPoint(points[i].x, points[i].y, (points[i].result) ? "#1aff00" : "#ff0000")
		}
	}
	 ...
	 */
	onClicked(successHandler, errorHandler) {
		let { canvas } = this

		$(canvas).click((e) => {
			let R = this.currentR
			if (!R || isNaN(R) || R === 0) {
				errorHandler("Cannot perform calculation without R set")
				return;
			}
			...
			console.log("Before: ", graphX)
			graphX = Math.round(graphX)
			console.log("After: ", graphX)
